<template>
  <div
    :class="loading ? 'content-center' : ''"
    class="container flex flex-row flex-wrap grow shrink gap-4 max-w-fit justify-center"
  >
    <h1 class="sm:hidden" v-if="!loading">
      <span class="capitalize">{{ selectedDifficulty }}</span> questions
    </h1>

    <div role="status" v-if="loading">
      <svg
        aria-hidden="true"
        class="w-8 h-8 mr-2 text-gray-200 animate-spin fill-blue-500"
        viewBox="0 0 100 101"
        fill="none"
...
export default class QuizView extends Vue {
  ...
  get selectedDifficulty(): string {
    return this.$route.params["difficulty"].toString();
  }
  ...
  async mounted(): Promise<void> {
    await this.triviaModule.getQuestions().then((_) => {
      this.loading = false;
    });
  }

  get questions(): TriviaQuestionModel[] {
    switch (this.selectedDifficulty.toLowerCase()) {
